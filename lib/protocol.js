...
class Protocol extends EventEmitter {
  ...
  ask(fn_num, msg_id, pkg_data) {
    switch (fn_num) {
      ...
      case 2:
        ...
        let data = JSON.parse(pkg_data[2].toString('utf8'));
        // 请求渲染文件
      ...
      case 7:
        ...
        // 应答文件内容
        this.emit('ansFileContent', msg_id, askcontent);
        break;

      default:
        this.emit('error', msg_id, 'unknow function num '+ fn_num);
        break;
    }
  }
  ...
  makeAskExt(msg_id, ext) {
    let str = ext.join(' ');
    return this.make(5, msg_id, [Buffer.from(str)]);
  }
